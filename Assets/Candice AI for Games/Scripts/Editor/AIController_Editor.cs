...
namespace ViridaxGameStudios.AI
{
    ...
    public class AIController_Editor : Editor
    {
        ...
        void DrawStatsGUI()
        {
            ...
            Draw(character.statStrength,window);

            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statIntelligence;
            Draw(character.statIntelligence,window);
            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statFaith;
            Draw(character.statFaith,window);
            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statHitPoints;
            Draw(character.statHitPoints,window);
            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statAttackDamage;
            Draw(character.statAttackDamage,window);
            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statAttackRange;
            Draw(character.statAttackRange,window);
            window = CreateInstance<AddModifierWindow>();
            window.stat = character.statMoveSpeed;
            Draw(character.statMoveSpeed,window);

        }
        public virtual void Draw(CharacterStat stat, EditorWindow window)
        {

            GUIStyle _style = EditorStyles.foldout;
            FontStyle previousStyle = _style.fontStyle;
            _style.fontStyle = FontStyle.Bold;
            GUIContent label1 = new GUIContent(stat.name);
            stat.showStat = EditorGUILayout.Foldout(stat.showStat, stat.name, _style);
            ...
            if (stat.showStat)
            {
                ...
                stat.baseValue = EditorGUILayout.FloatField(label, stat.baseValue);
                label = new GUIContent("Final Value", "The value after applying all modifiers.");
                EditorGUILayout.FloatField(label, stat.CalculateFinalValue());
                //GUIContent labe
                GUILayout.Space(4);
                //style.fontSize = 16;
                GUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                if (!stat.showModifiers)
                    GUILayout.BeginVertical("box");
                else
                ...
                if (stat.statModifiers.Count > 0)
                {
                    ...
                    stat.showModifiers = EditorGUILayout.Foldout(stat.showModifiers, "Modifiers");
                    if (stat.showModifiers)
                    {
